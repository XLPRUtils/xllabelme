...
import xllabelme.app
import xllabelme.config
import xllabelme.testing
...
def test_MainWindow_open_img(qtbot):
    ...
    win = xllabelme.app.MainWindow(filename=img_file)
    qtbot.addWidget(win)
...
def test_MainWindow_open_json(qtbot):
    ...
    for json_file in json_files:
        xllabelme.testing.assert_labelfile_sanity(json_file)

        win = xllabelme.app.MainWindow(filename=json_file)
        qtbot.addWidget(win)
        _win_show_and_wait_imageData(qtbot, win)
        win.close()
...
def test_MainWindow_open_dir(qtbot):
    ...
    win = xllabelme.app.MainWindow(filename=directory)
    qtbot.addWidget(win)
    _win_show_and_wait_imageData(qtbot, win)
    return win
...
def test_MainWindow_annotate_jpg(qtbot):
    ...
    config = xllabelme.config.get_default_config()
    win = xllabelme.app.MainWindow(
    ...
    xllabelme.testing.assert_labelfile_sanity(out_file)
